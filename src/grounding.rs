...
pub fn cache_dir() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("HANK_GROUNDING_CACHE_DIR") {
        return Some(PathBuf::from(path));
    }
    std::env::var_os("XDG_STATE_HOME")
        .map(PathBuf::from)
        .map(|p| p.join("hank").join("grounding"))
        .or_else(|| {
            std::env::var_os("HOME")
                .map(PathBuf::from)
                .map(|p| p.join(".local/state/hank/grounding"))
        })
}
...
pub fn max_age_secs() -> u64 {
    std::env::var("HANK_GROUNDING_MAX_AGE_SECS")
        .ok()
