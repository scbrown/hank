...
///
/// ```text
/// small repo      8,817 lines    45 ms    within budget
/// medium repo    33,094 lines   152 ms    OVER the 100 ms default
/// large repo     69,118 lines   313 ms    OVER by 3.1x
/// ```
///
/// The blast-radius scope is opt-in and off by default, so nothing is being disabled
/// today. The point is the DELIVERY: the moment a scope is enabled on a repo whose
/// walk exceeds the budget, the very first edit blows it — and before this split that
/// was indistinguishable from a clean pass. Keeping the two cases apart is what lets
/// the caller be loud about the one that means "unguarded" and quiet about the one
/// that means "nothing to guard".
