...
///
/// Records the notice as a marker file in `HANK_FAILOPEN_MARKER_DIR`, or the
...
pub fn first_notice_for_session(session: Option<&str>, kind: &str) -> bool {
    ...
    let marker = marker_dir.join(format!("hank-guard-failopen-{safe}-{kind_safe}"));
...
fn fail_open_marker_dir() -> PathBuf {
    std::env::var_os("HANK_FAILOPEN_MARKER_DIR").map_or_else(std::env::temp_dir, PathBuf::from)
}
...
fn prune_fail_open_markers(dir: &Path, now: SystemTime) {
    ...
    for entry in entries.flatten() {
        ...
        let is_marker = path
            ...
            .is_some_and(|name| name.starts_with("hank-guard-failopen-"));
        if !is_marker {
            continue;
        }
        let stale = entry
...
mod tests {
    ...
    fn fail_open_notice_fires_once_per_session() {
        ...
        for kind in ["config", "deadline-src/a.rs", "deadline-src/b.rs"] {
            ...
            let _ = std::fs::remove_file(
                fail_open_marker_dir().join(format!("hank-guard-failopen-{session}-{safe_kind}")),
            );
        }
    ...
    fn stale_fail_open_markers_are_pruned_but_fresh_markers_remain() {
        ...
        let stale = dir.join(format!("hank-guard-failopen-stale-{nonce}"));
        let fresh = dir.join(format!("hank-guard-failopen-fresh-{nonce}"));
