...
/// `aegis:verificationPoint`; hank hosts `Pag` and `Paa`, and carries the
/// remaining values so a projected policy round-trips rather than being dropped
/// for naming a point hank does not itself host.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum VerificationPoint {
    /// Pre-Action Gate — before dispatch. `hank hook pre-edit`.
    #[serde(rename = "PAG")]
    Pag,
    /// Action-Time Monitor — mid-flight. Not hosted by hank today.
    #[serde(rename = "ATM")]
    Atm,
    /// Post-Action Auditor — after the action, before the next. `hank hook
    /// post-edit`.
    #[serde(rename = "PAA")]
    Paa,
...
impl VerificationPoint {
    ...
    /// Whether hank's pre-edit guard is the host for this point. A rule
