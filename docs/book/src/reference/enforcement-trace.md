...
a second trace format. It additionally carries the four grounding fields. Its
`grounding_outcome` distinguishes `used`, `empty`, `transport-error`, `missing`,
`unresolved`, `stale`, and `unknown-scope`. Missing or unverifiable evidence is
`outcome: "unknown"`, never a fabricated success. Empty evidence is
`unsatisfied`; fresh used evidence is `satisfied`.
...
The cache defaults to `$XDG_STATE_HOME/hank/grounding` (or
`~/.local/state/hank/grounding`) and can be relocated by the harness with
`HANK_GROUNDING_CACHE_DIR`. Files are named `<sha256>.json`; the default
freshness ceiling is 300 seconds and the harness may set
`HANK_GROUNDING_MAX_AGE_SECS`.
